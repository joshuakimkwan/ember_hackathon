...
# 考虑mini_unit是否被满足
# 完善ma20和atr在df里面的体现
# 自动满足的订单在portfolio里面体现出来并计算PnL
# 做完一次交易以后，现有pnl是重新归0的，不合逻辑需要修改
# 需要考虑commission fee在不同情况下是收取多少的
# sell limit完善，并且实现update_portfolio里面对于sell limit的更新
# *****************************
...
def check_for_trades(df, portfolio, pair_or_coin, curr_cash, buy_expenditure):
    ...
    query_orders = query_order(pair_or_coin) # This is a list of dicts, each dict has keys OrderID, Ticker, Side, Quantity, Price, Status, Expire
    if query_orders['Success'] == False:
        pending_orders = []
    else:
        pending_orders = [order for order in query_orders["OrderMatched"] if order["Pair"] == pair_or_coin and order["Status"] == "PENDING"]
    ...
    if df["indicator"][1] == False \
        ...
        if spread < 0.001:
            ...
            quantity_buy_market = order["OrderDetail"]["Quantity"]
            price = order["OrderDetail"]["Price"]
            PnL = 0 # Since this is buy, nothing profitted yet
            # update_orders(pair_or_coin, "BUY", quantity_buy_market)
            update_portfolio(order_id, pair_or_coin, "BUY", quantity_buy_market, "MARKET", price, PnL) # TODO Use API to get current price and compute PnL
            df["indicator"][1] = True

        else:
            # BUY at limit order
            # place_order(pair_or_coin, "BUY", quantity, price=None, order_type=None)
...
#     order_file = "./intern/hachathon/orders.csv"
#     try:
...
def portfolio_file():
    portfolio_file = "./intern/hachathon/portfolio.csv"
    try:
...
if __name__ == "__main__":
    ...
    print("\n--- Getting Market Ticker (OPEN/USD) ---")
    ticker = get_ticker("OPEN/USD")
    if ticker:
        print(ticker.get("Data", {}).get("OPEN/USD", {}))
    ...
    while True:
        ...
        for ticker in list(info.get('TradePairs', {}).keys()):
            ...
            tdata = get_ticker(ticker)
            if tdata and tdata["Success"] == True:
                ticker_info = tdata.get("Data", {}).get(ticker, {})
            else:
                print(f"Failed to get data for {ticker}")
                continue

            file_name = ticker.replace("/", "_") + ".csv"
            path = os.path.join("./intern/hachathon/", file_name)

            # # If CSV does not exist, create and write header
            # if not os.path.exists(path):
            #     headers = ["Timestamp"] + list(ticker_info.keys())
            #     append_to_csv(path, headers)
            #     print(f"Created new CSV file: {file_name}")

            # 2. Populate CSV with data (see function tickers_to_csv or see the csv that has been populated)
            upload_info = [_get_timestamp()]
            upload_info += list(ticker_info.values())
            append_to_csv(path, upload_info)
            ...
            df['ATR'] = calculate_ATR(df, 14, "LastPrice")
            while len(df) > 2*50 - 1:
                df = df.iloc[1:]
